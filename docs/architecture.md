...
3. SIGTERM シグナルで graceful shutdown を実行するハンドラを登録
4. `bot.start` でイベントループを開始
...
- **並行処理制御**: メッセージごとに Thread を生成し、セマフォで最大同時実行数を 5 に制限
- **ファイル処理**: 添付ファイル（画像・PDF）をダウンロードしてローカルに保存し、Claude に渡す
- **リアクション管理**: 処理中はメッセージにリアクションを付与し、完了時に削除
- **Graceful Shutdown**: SIGTERM 受信時に新規メッセージの受付を停止し、実行中の処理の完了を最大 31 分待機
...
#### `PendingNotificationStore` (`lib/pending_notification_store.rb`)
再起動をまたいで送信する必要があるメッセージを一時保存する。

- 起動時に未送信メッセージがあれば自動送信
- 送信完了後にレコードを削除
...
SQLite の共通初期化処理。WAL モードとビジータイムアウト（5 秒）を設定する。
